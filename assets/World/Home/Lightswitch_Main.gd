...
func _ready():
	...
	if PlayerLog.home.lightswitch_checked:
		index = 1
	if PlayerLog.home.lightswitch_main_on:
...
func examine():
	...
	if get_parent().lightswitch_examined_while_off && get_parent().lightswitch_examined_while_on:
		PlayerLog.set_examined_and_signal("home_lightswitch")
...
func interact(_player):
	if !$Light2D.visible: # turn on light
		...
		$Light2D.show()
		PlayerLog.home.lightswitch_main_on = true
		...
		$AudioStreamPlayer.play()
		$Light2D.hide()
		PlayerLog.home.lightswitch_main_on = false
		index = 1
